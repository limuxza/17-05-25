...
function Linux.Create(config)
    ...
    CloseButton.MouseButton1Click:Connect(function()
        Linux:Notify({Title = "UI Closed", Content = "Press LeftAlt to reopen.", Duration = 5})
        LinuxUI:Destroy()
        ToggleUI:Destroy()
    end)
                    ...
                    for i, opt in pairs(Options) do
                        ...
                        OptBtn.MouseButton1Click:Connect(function()
                            if IsMulti then
                                ...
                                local currentOptAccentBar = OptBtn:FindFirstChild("OptAccentBar")
                                if index then
                                    table.remove(SelectedValues, index)
                                    OptBtn.TextColor3 = Color3.fromRGB(200, 200, 210)
                                    if currentOptAccentBar then
                                        TweenService:Create(currentOptAccentBar, tweenInfoShrink, {Size = UDim2.new(0, 0, 0, 20)}):Play()
                                    end
                                else
                                    table.insert(SelectedValues, opt)
                                    OptBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
                                    if currentOptAccentBar then
                                        TweenService:Create(currentOptAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 20)}):Play()
                                    end
                                end
                                UpdateSelectedText()
                                ...
                                if opt ~= SelectedValue then
                                    local oldSelectedValue = SelectedValue
                                    SelectedValue = opt
                                    ...
                                    for _, btn in pairs(OptionsHolder:GetChildren()) do
                                        if btn:IsA("TextButton") then
                                            local accentBar = btn:FindFirstChild("OptAccentBar")
                                            if btn.Text == tostring(SelectedValue) then
                                                btn.TextColor3 = Color3.fromRGB(255, 255, 255)
                                                if accentBar then
                                                    TweenService:Create(accentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 20)}):Play()
                                                end
                                            elseif btn.Text == tostring(oldSelectedValue) then
                                                btn.TextColor3 = Color3.fromRGB(200, 200, 210)
                                                if accentBar then
                                                    TweenService:Create(accentBar, tweenInfoShrink, {Size = UDim2.new(0, 0, 0, 20)}):Play()
                                                end
                                            end
                                        end
                                    end
    ...
    task.spawn(function()
        ...
        if DefaultTab and Tabs[DefaultTab] then
            for _, tab in pairs(Tabs) do
                ...
                if tab.TabAccentBar then
                    tab.TabAccentBar.Size = UDim2.new(0, 0, 0, 24)
                end
            end
            ...
            defaultTab.Button.BackgroundTransparency = 0.8
            ...
            if defaultTab.TabAccentBar then
                local tweenInfoExpand = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                TweenService:Create(defaultTab.TabAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 24)}):Play()
            end
        ...
        elseif tabOrder > 0 then
            ...
            for _, tab in pairs(Tabs) do
                ...
                firstTab.Button.BackgroundTransparency = 0.8
                ...
                if firstTab.TabAccentBar then
                    local tweenInfoExpand = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                    TweenService:Create(firstTab.TabAccentBar, tweenInfoExpand, {Size = UDim2.new(0, 4, 0, 24)}):Play()
            end
                AnimateTabSliders(1)
                CheckEmptyState()
            end
    ...
    if config.ConfigSave ~= false then
        ...
        SettingsTab.Section({Name = "Configuration", Container = 1})
        ...
        SettingsTab.Section({Name = "Audio", Container = 2})
        
        SettingsTab.Toggle({
            Name = "Enable Sound",
            Description = "Toggle UI sound effects",
            Default = Linux.SoundEnabled,
            Container = 2,
            Callback = function(state)
                Linux.SoundEnabled = state
            end
        })
    end
